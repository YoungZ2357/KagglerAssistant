...
def _merge_todos(current: list[dict] | None, update: list[dict] | None) -> list[dict]:
    """按 id upsert 合并待办列表。
    ...
    - update 中带已知 id 的项与既有项合并字段（如 status → done）；id 未知则按新建插入。

    这样容忍 LLM 一轮内多次调用待办工具（与 switch_mode 多写同源的问题）。
    """
